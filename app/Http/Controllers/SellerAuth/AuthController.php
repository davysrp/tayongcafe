...
class AuthController extends Controller
{
    ...
    {
        if (!\Auth::guard('seller')) return redirect()->back();
        auth()->guard('seller')->logout();  // Log out an administrator
    ...
    public function register(Request $request)
    {
        ...
        $mail = Mail::send('frontend__.auth.mail.welcome', $data, function ($message) use ($request) {
    ...
    {
        return view('frontend__.auth.register');
    }

    public function loginForm()
    {
        return view('frontend__.auth.login');
    }

    public function forgotPasswordForm()
    {
        return view('frontend__.auth.forgotpassword');
    }
    ...
    public function forgotPasswordSendLink(Request $request)
    {
        ...
        $checkAccount = Seller::whereEmail($request->email)->first();
        ...
        $mail = Mail::send('frontend__.auth.mail.resetpassword', $data, function ($message) use ($request) {
            $message->to($request->email)
    ...
    public function resetPasswordSendLink($token,Request $request)
    {
        ...
        if ($password) return view('frontend__.auth.setpassword', compact('password'));
    ...
    public function saveChangePassword(Request $request)
    {
        ...
        $findSellerAccount = Seller::whereEmail($request->email)->first();
...
//        try {
        ...
        $existingUser = Seller::where('google_id', $user->id)->orWhere('email', $user->email)->first();
...
//                if (auth()->guard('seller')->attempt(['google_id'=>$existingUser->google_id,'password'=>$user->id])) {
            ...
            if (auth()->guard('seller')->loginUsingId($newUser->id)) {
                ...
                $mail = Mail::send('frontend__.auth.mail.resetpassword', $data, function ($message) use ($user) {
