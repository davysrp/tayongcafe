                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        ...
                        {{-- <div class="col-md-3 p-1">
                            ...
                            {{-- <div id="payment-success-message" class="alert alert-success mt-2 d-none">
                                Payment Successful! 
                                <a id="view-invoice-btn" href="#" class="btn btn-sm btn-info">View Invoice</a>
                                <a id="download-invoice-btn" href="#" class="btn btn-sm btn-danger">Download PDF</a>
                            </div> --}}
...
{{-- 
                        ...
                        <div class="row mt-3">
                            <div class="col-md-12">
                                @foreach($category as $item)
                                    <button type="button" class="btn btn-primary btn-icon-split mr-1" id="category"
                                            data-id="{!! $item->id !!}">
    ...
    <!-- JavaScript for Handling Payment Confirmation -->
    <script>
        document.getElementById('confirm-payment').addEventListener('click', function () {
            let orderId = "{{ isset($order) ? $order->id : null }}"; // Get order ID
            if (!orderId) {
                alert('Invalid order ID');
                return;
            }
    
            // Simulate Payment Success (Replace with actual backend call if needed)
            setTimeout(() => {
                document.getElementById('payment-success-message').classList.remove('d-none');
    
                // Update the links dynamically
                document.getElementById('view-invoice-btn').href = "/invoice/view/" + orderId;
                document.getElementById('download-invoice-btn').href = "/invoice/download/" + orderId;
            }, 2000); // Simulated delay for effect
        });
        </script>
...
<!-- Modal -->
    <div class="modal fade" id="productVariant" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
         aria-hidden="true">
        ...
        <script>
            $(document).ready(function () {
                ...
                function updateRemoveQty(type, id, qty) {
                    $.post('{!! route('updateRemoveQty') !!}', {type: type, id: id, qty: qty}, function (data) {
                        if (data) {
                            orderItemList();
                        }
                    })
